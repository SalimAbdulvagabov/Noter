...
public class SheetViewController: UIViewController {
    ...
    public var shouldRecognizeSimultaneouslyWith = false
    ...
    public override func viewWillAppear(_ animated: Bool) {
        ...
        (MainRouter.shared.rootViewController as? StatusBarProtocol)?.statusBarStyle = .lightContent
        self.updateOrderedSizes()
    ...
    public override func viewDidDisappear(_ animated: Bool) {
        ...
        (MainRouter.shared.rootViewController as? StatusBarProtocol)?.statusBarStyle = .darkContent
    }
    ...
    @objc func panned(_ gesture: UIPanGestureRecognizer) {
        ...
        let minHeight: CGFloat = self.height(for: currentSize)
        ...
        // MARK: - TODO Здесь делаю пружинку
        if pointY > 0 {
            pointY = pointY*(40/(40 + pointY/2.5))
        }
        var newHeight = max(0, self.prePanHeight + (self.firstPanPoint.y + pointY))
...
extension SheetViewController: UIGestureRecognizerDelegate {
    ...
    /// Отключаю второй жест, было сделано чтобы на всплывашке работало удаление свайпом
